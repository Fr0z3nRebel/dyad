 ...
 *
 * SKIPPED: Quota limit has been removed (set to effectively unlimited).
 */

test.skip("free agent quota - full flow: mode availability, quota tracking, exceeded banner, switch to build", async ({
  po,
}) => {
...
test.skip("free agent quota - quota resets after 24 hours", async ({ po }) => {
  // Set up WITHOUT Dyad Pro - use test provider instead
  await po.setUp({ autoApprove: true });
  await po.importApp("minimal");
